 ...
 *
 * usage: ./hwdbclient [-h host] [-p port]
 *
 * each line from standard input is assumed to be a complete SQL query.  It is
 * sent to the Homework database server, and the results are printed on
 * the standard output
 */
...
#define USAGE "./hwdbclient [-h host] [-p port] [-l packets] [-b nlines]"
...
int main(int argc, char *argv[]) {
	...
	while (fetchline(inb) != NULL) {
		...
		if (log)
			printf(">> %s", query);
		count++;
		if (! rpc_call(rpc, query, n, resp, sizeof(resp), &len)) {
